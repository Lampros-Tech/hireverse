...
export function query(query) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (statement.length !== 2 && statement[1] !== "") {
            throw new Error("Invalid statement found in query. A Tableland query must be a single statement ending with a semicolon.");
        }
        ...
        const tablename = (_a = statement[0].match(/\b(?:FROM|JOIN|UPDATE|INTO)\s+(\S+(?:.\s)*)/i)) !== null && _a !== void 0 ? _a : []; // Find table name
        if (!(tablename && tablename[1])) {
            // If ID isn't a postive interger, throw error.
            throw new Error("No table name identifier found in query. Tableland does not support sql statements that do not" +
        ...
        if (!isPositiveInteger(tableId) && tablename[1] !== "system_table") {
            // If ID isn't a postive interger, throw error.
            throw Error("No ID found in query. Remember to add the table's ID after it's name. Ex; TableName_0000");
        }
        return yield tablelandCalls.query.call(this, query, tableId);
    });
}
